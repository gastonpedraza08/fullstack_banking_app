...
import {
  initiateGetAccontDetails,
  initiateAddAccontDetails,
  initiateUpdateAccontDetails
...
const AccountForm = (props) => {
  ...
  const processOnMount = useCallback(() => {
    if (propsRef.current === false) {
      const { email } = props;
      if (email) {
        props.dispatch(initiateGetAccontDetails());
  ...
  const handleUpdateAccount = (ifsc) => {
    ...
    if (!allFieldsEntered) {
      ...
      setErrorMsg('');
      props.dispatch(initiateUpdateAccontDetails(ifsc));
    }
  };
  ...
  const handleOnSubmit = (event) => {
    ...
    if (!allFieldsEntered) {
      ...
      if (selectedType === 'withdraw' && selectedAmount <= total_balance) {
        props.dispatch(
          initiateWithdrawAmount(account.id, selectedAmount)
        );
        setErrorMsg('');
      } else if (selectedType === 'deposit') {
        props.dispatch(
          initiateDepositAmount(account.id, selectedAmount)
        );
        setErrorMsg('');
      } else {
        setErrorMsg({
  ...
  const handleAddAccount = (account) => {
    const { account_nro, bank_name, ifsc } = account;
    props
      .dispatch(initiateAddAccontDetails(account_nro, bank_name, ifsc))
      .then(() => props.dispatch(initiateGetAccontDetails()));
  };

  const { selectedType } = props;
  const account_nro = account.account_nro ? maskNumber(account.account_nro) : '';
      ...
      <div className="edit-account-form  col-md-6 offset-md-3">
        ...
        <Form>
          ...
          <Form.Group controlId="acc_no">
            ...
            <span className="label-value">{account && account_nro}</span>
          </Form.Group>
